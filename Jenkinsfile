	...
	docker.withServer('tcp://192.168.100.160:2375') {
		...
		stage('Staging Environment') {
			...
			sshagent(['ssh-creds']) {
				...
				echo token
				sh "ssh -o StrictHostKeyChecking=no -l saicadm 192.168.100.80 \"sudo docker login -u test -e test@test.com -p ${token} && sudo docker push 172.30.122.20:5000/${registryTag}\""
			}
		}
